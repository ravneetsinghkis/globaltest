...
class DistilleryController extends Controller
{
    public function index()
    {
        ...
        return view('distillery.createdistillery',compact('countries','productCatalog'));
    }
    public function create(Request $request)
    {
        ...
        return redirect('/list-distillery')->with('success', 'Product Catalog record Added successfully.');
    }
    public function show(Categories $productCatalog)
    {
        //$distillerylist=Categories::where('type','Distillery')->paginate(10);
        if(isset($_GET['company_id'])){
            $company_id=$_GET['company_id'];
            $distillerylist = Categories::select('categories.id as distillery_id','categories.name as distillery_name', 'categories.story as distillery_story', 'categories.establishment as distillery_establishment', 'categories.logo as distillery_logo','companies.name as company_name')->join('companies', 'companies.id', '=', 'categories.company_id')->where('categories.type','Distillery')->where('companies.id',$company_id)->orderBy('categories.id', 'DESC')->paginate(10);
        }else{
            $distillerylist = Categories::select('categories.id as distillery_id','categories.name as distillery_name', 'categories.story as distillery_story', 'categories.establishment as distillery_establishment', 'categories.logo as distillery_logo','companies.name as company_name')->join('companies', 'companies.id', '=', 'categories.company_id')->where('categories.type','Distillery')->orderBy('categories.id', 'DESC')->paginate(10);
        }
        
        return view('distillery.listdistillery',compact('distillerylist'));
    }
    public function edit(Request $request, $id)
    {
        ...
        return view('distillery.editdistillery', compact('editbrands','countries','productCatalog','states'));
    }
    public function update(Request $request, $id)
    {
        ...
        $categories_update->socialmedia_link = $request->input('brandmedialink');
    ...
    public function destroy(Request $request,$id)
    {
        ...
        return redirect('/list-distillery')->with('success', 'Catalog record deleted successfully.');
    }
